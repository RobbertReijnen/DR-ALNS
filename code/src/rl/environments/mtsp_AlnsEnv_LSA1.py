...
from routing.mtsp.alns_mtsp.mtsp_env import mtspEnv
from routing.mtsp.alns_mtsp.destroy_operators import neighbor_graph_removal, random_removal, relatedness_removal
from routing.mtsp.alns_mtsp.repair_operators import regret_insertion
from routing.mtsp.alns_mtsp.initial_solution import compute_initial_solution

class mtspAlnsEnv_LSA1(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        base_path = Path(__file__).absolute().parents[4]

        nb_customers, dist_matrix_data, nr_vehicles, x_coords, y_coords = mtsp_helper_functions.read_input_mtsp(self.instance_file, self.instance)

        random_state = rnd.RandomState(SEED)
        state = mtspEnv([], nb_customers, dist_matrix_data, nr_vehicles, self.instance, SEED)
    ...
    def step(self, action, **kwargs):
        ...
        self.current_solution.graph = self.best_solution.graph = mtsp_helper_functions.update_neighbor_graph(candidate, candidate.routes, candidate.objective())
...
if __name__ == "__main__":
    ...
    env = mtspAlnsEnv_LSA1(get_parameters("mtspAlnsEnv_LSA1"))
    # print("Sampling random actions...")
    # env.sample()

    print('Start training')
    model = Trainer("mtspAlnsEnv_LSA1", "models").create_model()
    model.train()
    print("Training done")
