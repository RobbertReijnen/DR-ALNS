...
from routing.mtsp.alns_mtsp import initial_solution, destroy_operators, repair_operators

from alns.accept import SimulatedAnnealing
from alns.select import RouletteWheel
from alns.stop import MaxIterations

PARAMETERS_FILE = './configs/ALNS_mtsp_debug.json'
...
def run_algo(folder, exp_name, **kwargs):
    ...
    nb_customers, dist_matrix_data, nr_vehicles, x_coords, y_coords = mtsp_helper_functions.read_input_mtsp(instance_file, instance_nr)

    random_state = rnd.RandomState(seed)
    state = mtspEnv([], nb_customers, dist_matrix_data, nr_vehicles, instance_file, seed)
    ...
    helper_functions.write_output(folder, exp_name, instance_nr, kwargs['rseed'], kwargs['iterations'], solution.routes, best_objective, kwargs['instance_file'])
